...
import com.yuedi.service.fm.VideoService;
...
//	private static String utilUrl="http://www.mumfans.com/yuedi-resource/resources/video/";//正式服务器路径
	...
	private static String imageUrlAdd="D://yuedi-resource/resources/picture/";//本机路径
	private static String videolUrlAdd="D://yuedi-resource/resources/video/";//本机路径
	...
	public String add(Model model, HttpServletRequest request) {
		...
		model.addAttribute("parentId", parentId);
		model.addAttribute("action", "addVideo");
...
//							System.out.println(myFileName);
							...
							if(file.getName().equals("videoFile")){	//视频文件
								...
								File videoFile = createFile(videolUrlAdd+"delivery");	//创建视频文件夹
								File f = new File(videolUrlAdd+"delivery/"+fileName);	// 定义上传路径
								file.transferTo(f);
							}
							if(file.getName().equals("videoImgs")){	//图片
								video.setVideoImg("covert/"+covertName);
								File convertFile = createFile(imageUrlAdd+"covert");	//创建图片文件夹
								File f = new File(imageUrlAdd+"covert/"+fileName);	// 定义上传路径
								file.transferTo(f);	
							}
						}
					}
	...
	public String updateVideo(Model model,@ModelAttribute("form") Video video, HttpServletRequest request) throws IllegalStateException,
	IOException {
		String parentId = request.getParameter("parentId");
		String videoImg=request.getParameter("videoImg");
		String videoMp4=request.getParameter("videoMp4");
...
//							String fileName = null;
//							fileName = String.format("%1$s%2$s", UUID.randomUUID(), myFileName.substring(myFileName.lastIndexOf(".")));
							Video v = videoService.selectVideoByPrimaryKey(video.getId());
...
//								video.setVideoImg("covert/"+covertName);
								File convertFile = createFile(imageUrlAdd+"covert");	//创建图片文件夹
								File f = new File(imageUrlAdd+"covert/"+v.getVideoImg());	// 定义上传路径
								file.transferTo(f);	
							}
						}
					}
	...
	*/
	...
	public String updateForm(@PathVariable("id") Integer id, Model model,
		...
		try {
			...
			model.addAttribute("action", "updateVideo");
			model.addAttribute("imageUrl", imageUrl);
			model.addAttribute("videolUrl", videolUrl);
		} catch (Exception e) {
