...
public class PageInterceptor implements Interceptor {
    ...
    private String getOraclePageSql(MyPage<?> page, StringBuffer sqlBuffer) {  
       ...
       int offset = (page.getNumber() - 1) * page.getSize() + 1;  
       sqlBuffer.insert(0, "select u.*, rownum r from (").append(") u where rownum < ").append(offset + page.getSize());  
     ...
     */ 
    private void setTotalRecord(MyPage<?> page, MappedStatement mappedStatement, Connection connection) {  
    ...
    private String getCountSql(String sql) { 
       ...
       if(groupByIndex!=-1 && groupByIndex>index){
           return "select count(*) from (" + (orderBy==-1? sql:sql.substring(0,orderBy))+") "
                + "_allData"; 
       }else{
           return "select count(*) " + (orderBy==-1? sql.substring(index):sql.substring(index,orderBy));  
       }
    } 
